...
const addNotification = async ({ from, to, room}) => {
  const queryString = 'INSERT INTO notifications (from_id, to_id, room_id) VALUES ?';
  const queryArgs = [[[from, to, room]]];
  const data = await connection.promise().query(queryString, queryArgs);
  return data[0];
};
...
const getNotification = async (notificationId) => {
  const queryString = 'select notifications.*, rooms.name as room, users.username as username from notifications inner join rooms on notifications.room_id = rooms.id inner join users on notifications.from_id = users.id AND responded = 0 AND notifications.id = ?';
  const queryArgs = [notificationId];
  const data = await connection.promise().query(queryString, queryArgs);
  return data[0][0];
};
